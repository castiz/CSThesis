...
def add_features(df):
    ...
    for i in users:
        all_trans = df.loc[df['nameOrig'] == i]
        avg_trans = all_trans['amount'].mean()
        df.loc[df.nameOrig == i, 'avg_transaction'] = avg_trans
        #df['avg_transaction']['nameOrig' == i] = avg_trans
...
def data_split(df, fraud_prop):
    ...
    train_data, test_data, train_lbl, test_lbl = train_test_split(cleaned_data, labels, test_size=1/7.0, random_state=42) # had this set to 0 before
...
def experiments_nn(df):
    ...
    models3 = [
        ...
        (1024,1024,1024,512,2),
        (1024,1024,1042,1024,2)]
    ...
    for i in range(0, .5, .1):
        ...
        for param in models3:
            ...
            for i in range(1,epoch_limit):
                ...
                acc.append(str(round(score, 3)))
            end_time = time()
            line = '\t'.join([str(x), str(y), str(z), str(a), str(b), str(round(end_time - start_time, 1))]+acc)
            print(line)
            file.write(line + '\n')
...
def experiments(df):
    ...
    for i in range(0, .5, .1):
        ...
        start_time = time()
        accuracy, precision, recall, f1 = logisticRegression(train_data, test_data, train_lbl, test_lbl)
        end_time = time()
        ...
        start_time = time()
        accuracy, precision, recall, f1 = decision_tree(train_data, test_data, train_lbl, test_lbl)
        end_time = time()
        line = '\t'.join('decision_tree', str(i), '0', str(round(end_time - start_time, 1)), str(accuracy), str(precision), str(recall), str(F1) )
        file.write(line + '\n')

        start_time = time()
        accuracy, precision, recall, f1 = support_vector_machine(train_data, test_data, train_lbl, test_lbl)
        end_time = time()
...
def main():
    ...
    # df2 = add_features(df.head(10000))
    #
